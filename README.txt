...
- Demonstrate **idle CPU handling** and time quantum in Round Robin.
...
2. **SJF (Shortest Job First)**
   ...
   - Can be preemptive or non-preemptive (this project uses non-preemptive SJF).

3. **Round Robin (RR)**
   - Each process gets CPU for a fixed **time quantum**.
   - Preemptive, fair scheduling for all processes.
   - Shows idle times when CPU is free.

4. **Priority Scheduling**
   - Processes are scheduled based on **priority value**.
   - Lower number = higher priority (can be changed in code if desired).
